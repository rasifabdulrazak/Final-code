...
def admin_dashboard(request):
    if request.session.has_key('admin'):
        ...
        report_month = order_placed.objects.values('orderdate__month','orderdate__day','orderdate__year').filter(status='Delivered').annotate(Sum('sub_total')).order_by('-orderdate__month')[:7]
        report_date = order_placed.objects.values('orderdate__month','orderdate__day','orderdate__year').filter(status='Delivered').annotate(Sum('sub_total')).order_by('-orderdate__date')[:7]
        profit_loss = order_placed.objects.values('orderdate__month','orderdate__day','orderdate__year').filter(status='Canceled').annotate(Sum('sub_total')).order_by('-orderdate__date')[:7]
        category = Category.objects.values('name').annotate(Count('name'))
        order_status = order_placed.objects.values('status').annotate(Count('status'))
        product_brand = Products.objects.values('brand').annotate(Count('brand'))
        user_order = order_placed.objects.values('user').annotate(Count('user'))
        context = {
            'no_of_users': users,
    ...
    if request.session.has_key('admin'):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                Products(title=title,selling_price=selling_price,discounted_price=discounted_price,description=description,brand=brand,color=color,image=image,stock=stock,category_id=category,product_offer=offer).save()
...
def product_offer(request):
    if request.session.has_key('admin'):
        ...
        if request.method == "POST":
            ...
            data.discounted_price = (int(data.selling_price) - (int(data.selling_price) * int(data.product_offer) / 100))
            data.save()
...
def category_offer(request):
    if request.session.has_key('admin'):
        ...
        if request.method == "POST":
            ...
            for i in product:
                i.discounted_price = (int(i.selling_price) - (int(i.selling_price) * int(catoffer) / 100))
                i.save()
